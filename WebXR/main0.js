...
function init() {
    ...
    const geometry = new THREE.BoxGeometry(); //it must have a shape, but be careful with the size
                                                         //of the object. If it is too big, the faces 
                                                         //may be too far 
                                                         //to be seen.
    const material = new THREE.MeshBasicMaterial( {color: 0x0011ff} ); //and lets give it some color

    const cube = new THREE.Mesh(geometry, material);//now we create an object with the given gemoetry and material

    //the object exist, but where is it?
    cube.position.set(0, 0, -2); //we put that in he center (x,y) and two meters back (z)
